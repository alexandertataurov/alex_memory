...
.PHONY: help setup hooks run daemon test test-fast coverage lint format format-check typecheck check lock-check deps audit verify health db-check db-backup repair-dry-run graph-parity profile-acceptance profile-review person-intelligence-benchmark docs docs-check changes tasks review codex-hooks-check codex-check
...
help:
	@printf '%s\n' 'Alex Memory development commands:' \
	  ...
	  '  make profile-review     Run the resumable AM-122 owner review.' \
...
profile-review:
	$(PYTHON) scripts/dev_tools.py profile-review
