...
- [x] [Notion / Sequence 31] AM-127 — Reconcile missing aliases into canonical
  ...
  - A direct-chat title now becomes a canonical person's alias only after an
    exact peer-ID or unique username match and only with a single alias owner.
    Conflicts preserve manual aliases and enter the existing review path; no
    automatic merge occurs.
  - Verification: focused operational identity regressions and `make check`
    passed. No schema, live repair, migration, replay, backfill, provider, or
    graph work changed.
...
- [x] [Notion / Sequence 60] Add bounded Evidence pagination/count instead of
  ...
  - Evidence now shows `showing N / total`, displays at most 16 exact sources
    per page, and supports keyboard previous/next navigation without writes.
  - Verification: full repository gate passed (384 tests, Ruff, formatting,
...
- [x] AM-074 [P1] [Data repair] — Add a safe, resumable derived-state repair/backfill command for the existing live database after logic fixes.
  ...
  - Progress 2026-08-29: fixture-only targeted context repair now selects only
  ...
  - Remaining lifecycle boundary: no task-lifecycle repair is authorized until
